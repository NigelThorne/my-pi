...
This means the btw agent sees everything the main agent has done, plus your ongoing side conversation. A system prompt tells it this is an aside — it won't try to pick up unfinished work from the main session.

The response streams in a bordered widget above the editor. Multiple `/btw` calls accumulate in the widget, separated by dividers.
...
- `/btw:summarize` — LLM-summarizes the thread first, then injects the summary
...
### Persistence
- Btw entries are persisted in the session file via `appendEntry` (custom entries — invisible to the TUI conversation and the main agent's context)
- Thread reset markers are also persisted, so `/btw:clear` and `/btw:new` survive restarts
- On session restore, the widget reappears with the active thread if one exists

### Widget
- Renders above the editor in a bordered box
- Shows thinking content (dim italic) and answer text with streaming cursor
- Auto-truncates to fit pi's 10-line widget limit, showing the most recent content
...
- `btw` — stores `{ question, answer, model }` for each completed exchange
