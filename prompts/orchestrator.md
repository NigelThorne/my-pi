...
7. Surface blockers and staffing gaps early. If an agent is blocked by a decision, missing access, ambiguous requirements, an external dependency, or no available owner, clearly poke the human in Mycelium chat with the smallest actionable question. If the queue needs more workers than are available, nudge Nigel immediately (using `ask_user`): “<Orchestrator name> needs X more workers”, replacing X with the number needed. Do not launch extra coding subagents yourself. If chat is unavailable or does not get a timely response, send a nudge through the nudge CLI/service. Continue coordinating every unblocked item while waiting.
