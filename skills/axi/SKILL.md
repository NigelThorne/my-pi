...
When you want to inspect, filter, extract, or convert AXI output, prefer `toonq` instead of falling back to JSON and `jq`. `toonq` is jq-for-TOON and accepts jq-like filters directly against TOON streams:

```sh
# install once if missing
cargo install toonq

# inspect/filter/extract TOON without converting to JSON
...
# convert only when a downstream tool genuinely requires JSON
...
- **Make the error self-correcting in one turn.** The agent's deterministic next move after an unknown-flag error is to run `<command> --help` (e.g. `tasks list --help`) — so fold that lookup into the error: list the valid flags inline, or print the command's concise `--help` block directly beneath it.
...
- **Path repair**: setup commands should check existing hooks and update the executable path if it has changed
...
- **Token-budget-aware**: this context loads on _every_ session — ruthlessly minimize it.
- **Lifecycle capture**: use session-end hooks to capture what happened so future session-start context gets richer over time
...
- **Strip live state**: a skill is static, so omit dynamic data that only the hook can show
- **Non-interactive commands**: rewrite command examples to a form the agent can run without a global install
...
- **Document both paths**: in your README, present the hook and the skill as two ways to achieve the same thing
...
- **Parameterize dynamic values**: use placeholders like `<id>` or `"<title>"` instead of guessing a concrete value that may mislead the agent
...
- **Resolve errors**: on errors, suggest the specific command that fixes the problem
